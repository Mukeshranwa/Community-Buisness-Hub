...
public class ProductDAO {
    ...
    public ResultSet getCurrentStockInfo() {
        try {
            String query = """
                    ...
                    ON currentstock.productcode=products.productcode ORDER BY products.pid;
                    """;
            resultSet = statement.executeQuery(query);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
    ...
    public ResultSet getSalesSearch(String text) {
        try {
            String query = "SELECT salesinfo.salesid,salesinfo.productcode,salesinfo.quantity,revenue,soldby FROM salesinfo " +
                "WHERE salesinfo.productcode LIKE '%"+text+
                "%' OR salesinfo.soldby LIKE '%"+text+
                "%' ORDER BY salesid;";
            resultSet = statement.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public ResultSet getPurchaseSearch(String text) {
        try {
            String query = "SELECT purchaseinfo.purchaseID,purchaseinfo.productcode,quantity,totalcost FROM purchaseinfo " + 
                    "WHERE purchaseinfo.productcode LIKE '%"+text+
                    "%' ORDER BY purchaseid;";
            resultSet = statement.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
